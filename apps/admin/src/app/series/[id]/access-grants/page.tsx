...
export default function SeriesAccessGrantsPage() {
  ...
  async function loadPage() {
    setLoading(true);
    try {
      const [seriesList, grantsData] = await Promise.all([
        apiGet<any[]>("/series?admin=true"),
        apiGet<Grant[]>(`/series/${id}/access-grants`),
      ]);
      const series = seriesList.find((s: any) => s.id === id);
  ...
  const runSearch = useCallback(async (q: string) => {
    ...
    try {
      const results = await apiGet<UserResult[]>(
        `/series/${id}/access-grants/users/search?q=${encodeURIComponent(q)}`
      );
  ...
  async function handleGrant() {
    ...
    setGrantError("");
    try {
      await apiPost(`/series/${id}/access-grants`, {
  ...
  async function loadGrants() {
    const data = await apiGet<Grant[]>(`/series/${id}/access-grants`);
    setGrants(data);
  }
  ...
  async function handleRevoke(userId: string) {
    setRevoking(userId);
    try {
      await apiDelete(`/series/${id}/access-grants/${userId}`);
  ...
  return (
    <AdminShell>
      <div className="mb-6">
        <Link href="/series" className="text-sm text-charcoal/50 hover:text-soil">
          ← Back to Series
        </Link>
        <div className="mt-3 flex items-start justify-between">
          <div>
            ...
            {seriesTitle && (
              <p className="text-sm text-charcoal/60 mt-1">{seriesTitle}</p>
            )}
          </div>
